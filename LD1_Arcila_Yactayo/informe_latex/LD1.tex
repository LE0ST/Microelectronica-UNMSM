...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.64\textwidth]{figuras/fig1_cursores_ltspice.png}
\caption{Medición directa en LTspice de los voltajes críticos $V_{IL}$ y $V_{IH}$ para el caso óptimo $\beta = 2.5$ ($V_M \approx 0.50$~V) mediante cursores situados en los puntos de ganancia unitaria $d(V_{out})/d(V_{in}) = -1$.}
...
\par\noindent\textbf{Rpta:} Las mediciones ratifican que NAND2 es más rápida ($t_p = 7.910$~ps vs $11.862$~ps) y compacta ($\sum W = 1620$~nm vs $2160$~nm). La industria prefiere NAND porque conecta en serie transistores NMOS (alta movilidad), duplicando un $W_n$ pequeño; la NOR apila PMOS (menor movilidad), forzando anchos $W_p$ muy grandes que penalizan área y capacidad de entrada.
